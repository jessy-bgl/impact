...
/**
 * Dev-only datasets, rendered when the picker of `DevEmissionsHistory` selects
 * one. They exist so the chart, the filters, the point selection and
 * the empty states can be seen without waiting weeks for real snapshots to
...
/** The four states the screen can be in, plus the real data it defaults to. */
...
/**
 * Both the history and the profile completion, because the empty states are
 * told apart by the profile: an empty history under an incomplete profile means
 * tracking has not started, under a complete one that the first snapshot is on
 * its way. Overriding only the history could never reach the latter.
 */
...
> = {
  ...
  firstSnapshotPending: { history: [], profileCompleted: true },
