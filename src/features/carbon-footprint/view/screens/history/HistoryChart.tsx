...
export const HistoryChart = ({
  ...
  // Everything that places the y axis, shared by the plot and the pinned axis.
  const axisProps = {
    height: chartHeight,
    yAxisLabelWidth,
    yAxisThickness,
  ...
  // range between them only, so these two points scale the axis exactly like
  // the whole series — without drawing it a second time.
  const byValue = [...data].sort((a, b) => a.value - b.value);
  const axisData = [byValue[0], byValue[byValue.length - 1]];
    ...
    <View style={{ paddingHorizontal: horizontalPadding }}>
      ...
      <View style={{ width: visibleWidth, overflow: "hidden" }}>
        ...
        {/* The y axis has to stay put while the plot scrolls under it, but
            ...
            lineless chart here, clipped to the axis column: the same axis
            props and extremes guarantee the same vertical geometry. */}
        <View
          pointerEvents="none"
          style={{
            position: "absolute",
