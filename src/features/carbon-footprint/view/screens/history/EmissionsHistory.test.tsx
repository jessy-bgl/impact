...
describe("EmissionsHistory", () => {
  ...
  it("announces the first snapshot while a complete profile has none yet", async () => {
    await renderHistory([], { profileComplete: true });

    expect(
      screen.getByText(emissions.history.firstSnapshotPending.title),
    ).toBeOnTheScreen();
    expect(
      screen.queryByText(emissions.history.incompleteProfile.title),
    ).not.toBeOnTheScreen();
  });
