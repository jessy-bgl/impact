...
describe("buildFootprintsHistoryViewModel", () => {
  ...
  describe("trend", () => {
    ...
    it("has no percentage when growing from zero", () => {
      const { variation } = buildFootprintsHistoryViewModel(
    ...
    it.each([
      ...
      "is stable on %s smaller than the displayed precision",
      (_, direction) => {
        const before = 10200;
        const after = before + direction * (HISTORY_DELTA_PRECISION_KG / 2 - 1);
