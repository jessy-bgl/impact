...
/**
 * Deltas are rounded to the 0,01 t the history displays: a smaller move would
 * show as `+0,00 t` under an up or down arrow, for a change the user cannot see.
 */
export const HISTORY_DELTA_PRECISION_KG = 10;

export type HistoryVariation = {
  /** The point the current value is compared against. */
  fromDate: string;
  /** Rounded to `HISTORY_DELTA_PRECISION_KG`. */
  deltaKg: number;
  /**
   * Absolute, rounded. The sign lives in `trend`, as in
   * `FrenchAverageComparisonViewModel`. `null` when the reference is zero: a
   * category that was at zero has no meaningful ratio to grow from.
   */
 ...
 * `firstSnapshotPending` is a complete profile whose first snapshot has not
 * been recorded yet.
 */
export type HistoryEmptyStateVariant =
  | { name: "incompleteProfile" }
  | { name: "firstSnapshotPending" }
  | { name: "singleSnapshot"; value: number };
...
const buildVariation = (
  ...
  // `|| 0` folds the `-0` that rounding a small drop produces.
  const deltaKg =
    Math.round((to.value - from.value) / HISTORY_DELTA_PRECISION_KG) *
      HISTORY_DELTA_PRECISION_KG || 0;
  ...
  return {
    ...
    percentage:
      from.value === 0
        ? null
        : Math.round((Math.abs(deltaKg) / from.value) * 100),
