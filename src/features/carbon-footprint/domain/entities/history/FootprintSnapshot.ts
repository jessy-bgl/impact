 ...
 *
 * Returns the **same reference** when there is nothing to write, which is how
 * the caller knows to skip the store update: without it, every app start would
 * append an identical point and flatten the chart into noise.
 */
export const upsertSnapshot = (
  ...
  const withoutToday =
    last?.date === snapshot.date ? history.slice(0, -1) : history;

  const previous = withoutToday[withoutToday.length - 1];
  // Also covers the case where the day's value is edited back to the previous
  // day's: the now-redundant entry is dropped rather than duplicated.
