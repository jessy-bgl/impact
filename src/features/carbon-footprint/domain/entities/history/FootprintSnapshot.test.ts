...
describe("upsertSnapshot", () => {
  ...
  it("has nothing to write for a new day with unchanged footprints", () => {
    const history = [snapshot("2026-03-12", 100)];

    expect(upsertSnapshot(history, snapshot("2026-05-14", 100))).toBeNull();
  });

  it("has nothing to write when re-recording the day with the same value", () => {
    const history = [snapshot("2026-03-12", 100), snapshot("2026-05-14", 90)];

    expect(upsertSnapshot(history, snapshot("2026-05-14", 90))).toBeNull();
  });
